...
----------------------------------------
Pattern Priority Strategy — Tracks ALL possible patterns simultaneously.
The FIRST pattern to complete (reach entry signal) wins and all others are discarded.

Philosophy:
    Don't track just one SELL and one BUY pattern. Track EVERY possible pattern
    that starts (any sequence of consecutive red/green candles). When ANY pattern
    completes and generates an entry signal, that's the one we take, and all
    other patterns are reset.

Pattern Detection Logic:
    - Track multiple SELL patterns in a list (each with its own state)
    - Track multiple BUY patterns in a list (each with its own state)
    - On every candle, update ALL active patterns
    - First pattern to reach PHASE5 and generate entry signal → WINS
    - When a pattern wins, reset ALL other patterns (both SELL and BUY)
"""
...
class PatternPriorityStrategy(ChangeOfDirectionStrategy):
    """
    Pattern Priority Strategy — tracks all possible patterns, first to complete wins.
    """
